		...
		if (animation)
			...
			if (pressed) {
				...
				if (isBoll) {
					...
					System.out.println("˝ÚÓ ¯‡ " + selectedBollButton.getRow()	+ " " + selectedBollButton.getColumn());
				}
				...
				if ((bollSelected && !isBoll) || (bollSelected && moveButton.isSmallBoll())) {
					...
					Point start = new Point(selectedColumn, selectedRow); // H‡˜‡Î¸Ì‡ˇ
																			// ÚÓ˜Í‡
